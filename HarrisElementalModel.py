...
def getCapacity(csRep):
    sumW = np.sum(csRep,1) #summed weights for each stimuli
    capacity = max(sumW) # Find maximum to include all weights
    return capacity


# trialReps = matrix for CS-US connections for each item in either legendR or legendV
# Inspired by HMS
def getTrialReps(noPres,csRep,trials,sparse,capacity,gain,noElems,L):
    trialReps = {}
    cutoffPres = {}
    for i in range(noPres):
        tmp = np.sum(csRep[trials[i,:]==1, :], axis=0)
        tmp3 = np.flipud(np.sort(tmp))           
        cutoffPres[i] = max(np.multiply((np.cumsum(tmp3) > capacity),tmp3))
        rep = (gain * np.multiply(tmp>=cutoffPres[i],tmp)) + np.multiply(tmp<cutoffPres[i],tmp) 
        trialReps[i]= np.multiply(sparse,np.tile(rep,(noElems,1)))
    return trialReps    
...
def trainHEM(groups,randomTrialsValue,noElemsVariableList,gainVariableList,
    ...
    for n in range(len(groups)):
        ...
        for i in range(runs):
            ...
            try:
                capacity = getCapacity(csRep) * capacityFactor
            except ValueError:
                continue
            ...
            for p in range(noPhases):
                ...
                trialRepsR = getTrialReps(len(trialsR),csRep,trialsR,sparse,capacity,gain,noElems,L)
                trialRepsV = getTrialReps(len(trialsV),csRep,trialsV,sparse,capacity,gain,noElems,L)
...
if __name__ == "__main__":
    ...
    with SplashScreen( root, 'splashImage.jpg', 1.0):
        root.title("HEM SIMULATOR ver 1.0")
